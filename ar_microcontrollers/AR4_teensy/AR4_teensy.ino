...
void stateTRAJ()
{
  ...
  while (STATE == STATE_TRAJ)
  {
    ...
    if (received == '\n')
    {
      ...
      else if (function == "SG") // Set Gripper Position
      {
        int idxValue = inData.indexOf('A');
        int gripperValue = inData.substring(idxValue + 1).toInt();
        gripperServo.write(gripperValue);
        gripperPosition = gripperValue;
        // Acknowledge command
        String msg = String("SGOK");
        Serial.println(msg);
      }
      else if (function == "GG") // Get Gripper Position
      {
        // Since standard servos don't provide position feedback,
        // we return the last commanded position.
        String msg = String("GG") + "A" + gripperPosition;
        Serial.println(msg);
      }
